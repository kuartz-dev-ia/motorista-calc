...
class EventosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for ((chip, categoria) in chips) {
            chip.setOnClickListener {
                selecionarCategoria(chip, categoria)
            }
        }

        selecionarCategoria(
            findViewById(R.id.chipShow),
            "Show/Festival"
        )

        findViewById<TextView>(R.id.btnExtrairImagem).setOnClickListener {
            escolherImagem()
        }

        findViewById<TextView>(R.id.btnSalvarEvento).setOnClickListener {
            salvar()
        }
    }

    private fun selecionarCategoria(
        selecionado: TextView,
        categoria: String
    ) {
        ...
        for ((chip, _) in chips) {
            ...
            chip.background =
                ContextCompat.getDrawable(this, backgroundRes)

            chip.setTextColor(
                if (chip == selecionado) {
                    Color.parseColor("#08131A")
                } else {
                    Color.parseColor("#8B96AC")
                }
            )
        }
    }
    ...
    override fun onActivityResult(
        ...
        if (
            requestCode != CODIGO_ESCOLHER_IMAGEM ||
        ...
        try {
            ...
            recognizer.process(inputImage)
                .addOnSuccessListener { visionText ->
                    ...
                    val endereco =
                        EventoTextParser.extrairEndereco(texto)

                    val nome =
                        EventoTextParser.extrairNome(
                            texto,
                            endereco
                        )

                    val data =
                        EventoTextParser.extrairData(texto)

                    val horario =
                        EventoTextParser.extrairHorarioInicio(texto)
                    ...
                    horario?.let {
                        edtHorarioInicio.setText(it)
                    }

                    Toast.makeText(
                        this,
                        "Dados extraídos — confira e ajuste se precisar",
                        Toast.LENGTH_LONG
                    ).show()
                }
                .addOnFailureListener { erro ->
                    ...
                    Toast.makeText(
                        this,
                        "Erro ao ler o texto da imagem: ${erro.message}",
                        Toast.LENGTH_LONG
                    ).show()
                }
    ...
    private fun salvar() {
        ...
        val endereco =
            edtEndereco.text.toString().trim()
        ...
        if (
            ...
            Toast.makeText(
                this,
                "Preencha pelo menos o nome e o endereço (extraia de uma imagem ou digite)",
                Toast.LENGTH_LONG
            ).show()
